...
export default function MainBody(props) {
  ...
  const updateTrending = useCallback(async () => {
  ...
  const updatePopular = useCallback(async () => {
    props.setProgress(10);
  ...
  const handleSearchKeyDown = (event) => {
    if (event.key === "Enter") {
      handleSearchSubmit(event); // Call the search submit function
    }
  };
  ...
  }, [updateTrending, updateVideos, updatePopular]);
          ...
          <div className="search-box">
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                onKeyDown={handleSearchKeyDown} // Added onKeyDown event
              />
